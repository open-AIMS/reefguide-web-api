...
export const JobTypeConfigSchema = z.object({
  ...
  desiredMinCapacity: z.number().min(0, 'Minimum capacity must be non-negative'),
  desiredMaxCapacity: z.number().min(0, 'Maximum capacity must be non-negative'),
...
export function createEnvVarsSchema(): z.ZodObject<any> {
  ...
  Object.values(JobType).forEach(jobType => {
    ...
    Object.entries(JOB_TYPE_ENV_FIELDS).forEach(([envSuffix, configField]) => {
      ...
      if (['MIN_CAPACITY', 'MAX_CAPACITY', 'SCALE_THRESHOLD', 'COOLDOWN'].includes(envSuffix)) {
        envSchema[envVarName] = z.string()
          .transform(val => {
 ...
 */
function buildJobTypeConfig(env: Record<string, any>, jobType: string): JobTypeConfig {
  try {
    return {
    ...
    throw new Error(`Failed to build configuration for job type ${jobType}: ${error}`);
  }
}
